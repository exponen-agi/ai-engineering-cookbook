 ...
 *
 * Installs the Skill Review skill (an Agent Skills / SKILL.md open-standard
 * skill, https://agentskills.io) into any compatible agentic tool, plus the
 * deterministic gate that makes its first step real: `scripts/check-skills.js`.
 *
 * The skill teaches an agent to review a SKILL.md before it is installed,
 * published or trusted — specification conformance, portable vs vendor-only
 * frontmatter, and characters a human reviewer cannot see. The gate is the part
 * that does not depend on anyone's judgement, so it is installed by default and
...
if (opts.help) {
  process.stdout.write(
    [
      "install-skill-review — install the Skill Review skill + the SKILL.md gate",
      "",
      "Usage:",
      "  install-skill-review [options]",
      "",
      ...
      "",
      "Examples:",
      "  # Claude Code, project scope, skill + gate (default)",
      "  install-skill-review",
      "",
      "  # Just the skill for Cursor, no gate tooling",
      "  install-skill-review --tool cursor --skill-only",
      "",
    ].join("\n"),
  );
  process.exit(0);
...
const CHECKER_DEST = path.join(cwd, "scripts", "check-skills.js");
...
// --- Run ----------------------------------------------------------------
log(
  `Installing Skill Review for ${profile.label}${opts.user ? " (user-global skill)" : ""}${opts.dryRun ? " (dry-run)" : ""}`,
);
...
if (opts.tool === "claude") {
  ...
  log("  2. Try `/skill-review` (or ask the agent to use the skill-review skill).");
  ...
  log(`  2. Ask your agent to "use the skill-review skill to vet this skill before I install it".`);
}
if (!opts.skillOnly) {
  log("  3. Check the skills you already have installed:");
  log(`       node scripts/check-skills.js ${profile.skillsDirProject || "<your skills dir>"}`);
  log("  4. Optional — add it to CI:  node scripts/check-skills.js skills --strict");
}
